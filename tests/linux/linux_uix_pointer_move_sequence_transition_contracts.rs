...
//! UIX 纯 pointer_move 序列 transition 的输入、结果与安全边界契约回归。
...
fn envelope(data: Value) -> Value {
    json!({
        ...
        "app": "app",
        "verb": "apply",
        "capability": "ui.input.pointer.move.sequence.transition@1",
...
#[test]
fn schemas_accept_unique_missing_and_unsettled_move_sequence_results()
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let input_schema = schema(include_str!(
        "../../contracts/v1/uix-pointer-move-sequence-transition-input.schema.json"
    ))?;
    assert_valid(&input_schema, &input)?;

    let result_schema = schema(include_str!(
        "../../contracts/v1/uix-pointer-move-sequence-transition-result.schema.json"
    ))?;
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    for invalid in [
        ...
        json!({
            "coordinateSpace": "client-logical-px",
            "moves": [{ "x": 65_535.1, "y": 2 }, { "x": 3, "y": 4 }],
            "postcondition": postcondition.clone()
        }),
        json!({
            "coordinateSpace": "client-logical-px",
            "moves": [{ "x": 1, "y": null }, { "x": 3, "y": 4 }],
            "postcondition": postcondition.clone()
        }),
        json!({
            "coordinateSpace": "client-logical-px",
            "moves": [{ "x": 1, "y": 2, "kind": "pointer_move" }, { "x": 3, "y": 4 }],
            "postcondition": postcondition.clone()
        }),
        json!({
            "coordinateSpace": "client-logical-px",
        ...
        json!({
            "coordinateSpace": "client-logical-px",
            "moves": [{ "x": 1, "y": 2 }, { "x": 3, "y": 4 }],
            "postcondition": { "selector": { "role": "status" }, "condition": "stable" }
        }),
    ] {
        assert!(
            jsonschema::draft202012::validate(&input_schema, &invalid).is_err(),
            "非法 pointer move sequence transition 输入不应通过: {invalid}"
        );
    }
    Ok(())
}
...
fn confirmation_source_order_is_checked_when_transition_module_exists() {
    let Some(module) = source("src/modules/linux_uix_pointer_move_sequence_transition.rs") else {
        eprintln!("跳过：pointer move sequence transition Module 尚未出现");
        return;
    };
    let Some(confirmation) = module.find("if !confirmed") else {
        panic!("Module 必须先检查 confirmation");
    };
    let Some(input) = module.find("UixPointerMoveSequenceTransitionInput::parse") else {
        panic!("Module 必须解析 pointer move sequence transition 输入");
    };
    let Some(provider) = module.find(".perform(session_id") else {
        panic!("Module 必须通过 provider port 执行移动序列");
    };
    assert!(confirmation < input && input < provider);

    let Some(adapter) = source("src/adapters/linux/uix_agent_pointer_move_sequence_transition.rs")
    else {
        eprintln!("跳过 Adapter 顺序断言：pointer move sequence transition Adapter 尚未出现");
        return;
    };
    let Some(wait) = adapter.find("execute_wait_from_baseline") else {
        panic!("Adapter 必须在完整移动序列后执行 semantic wait");
    };
    let Some(sequence) = adapter.find("execute_sequence") else {
        panic!("Adapter 必须先执行完整移动序列");
    };
    assert!(sequence < wait);
}
...
fn partial_ambiguity_and_unknown_results_prohibit_automatic_retry_when_implemented() {
    let Some(adapter) = source("src/adapters/linux/uix_agent_pointer_move_sequence_transition.rs")
        ...
        eprintln!("跳过：pointer move sequence transition Adapter 尚未出现");
        return;
    };
    for marker in [
...
#[test]
fn result_security_contract_forbids_other_input_and_host_semantics() {
    let schema =
        include_str!("../../contracts/v1/uix-pointer-move-sequence-transition-result.schema.json");
    let adapter = source("src/adapters/linux/uix_agent_pointer_move_sequence_transition.rs");
    let module = source("src/modules/linux_uix_pointer_move_sequence_transition.rs");
    for implementation in [Some(schema), adapter.as_deref(), module.as_deref()] {
        let Some(implementation) = implementation else {
            continue;
        };
        // 测试夹具可拥有宿主连接字段，但生产实现不能发布这些身份或使用桌面后门。
    ...
    for marker in [
        ...
        "\"clickSupported\": { \"const\": false }",
        "\"keyPressSupported\": { \"const\": false }",
        "\"independentButtonOwnership\": { \"const\": false }",
        "\"interpolationSemantics\": { \"const\": false }",
...
fn unconfirmed_cli_fails_before_target_input_or_provider_discovery() {
    let error = match cli::run(vec![
        ...
        "apply".to_owned(),
        "--capability".to_owned(),
        "ui.input.pointer.move.sequence.transition@1".to_owned(),
        "--target".to_owned(),
        "sessionId=not-a-target".to_owned(),
        "--input".to_owned(),
        "/definitely/not/read-before-confirmation.json".to_owned(),
    ]) {
        Ok(_) => panic!("未确认 pointer move sequence transition 必须优先失败"),
        Err(error) => error,
    };
