...
//! UIX 按键序列 transition 的输入、结果与安全边界契约回归。
...
fn envelope(data: Value) -> Value {
    json!({
        ...
        "app": "app",
        "verb": "apply",
        "capability": "ui.input.key.sequence.transition@1",
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let input_schema = schema(include_str!(
        "../contracts/v1/uix-key-sequence-transition-input.schema.json"
    ))?;
...
#[test]
fn input_schema_rejects_unknown_null_invalid_press_and_postcondition()
-> Result<(), Box<dyn std::error::Error>> {
    let input_schema = schema(include_str!(
        "../contracts/v1/uix-key-sequence-transition-input.schema.json"
    ))?;
    ...
    for invalid in [
        ...
        assert!(
            ...
            "非法 key sequence transition 输入不应通过: {invalid}"
        );
    }
    Ok(())
}
...
fn confirmation_source_order_is_checked_when_transition_module_exists() {
    let Some(module) = source("src/modules/linux_uix_key_sequence_transition.rs") else {
        eprintln!("跳过：key sequence transition Module 尚未出现");
        return;
    };
    let Some(confirmation) = module.find("if !confirmed") else {
        panic!("Module 必须先检查 confirmation");
    };
    let Some(input) = module.find("UixKeySequenceTransitionInput::parse") else {
        panic!("Module 必须解析 key sequence transition 输入");
    };
    let Some(provider) = module.find(".perform(session_id") else {
        panic!("Module 必须通过 provider port 执行按键序列");
    };
...
fn partial_ambiguity_and_unknown_results_prohibit_automatic_retry_when_implemented() {
    let Some(adapter) = source("src/adapters/linux/uix_agent_key_sequence_transition.rs") else {
        eprintln!("跳过：key sequence transition Adapter 尚未出现");
        return;
    };
    for marker in [
    ...
    let Some(module) = source("src/modules/linux_uix_key_sequence_transition.rs") else {
        eprintln!("跳过 Module 不确定性断言：key sequence transition Module 尚未出现");
        return;
    };
    for marker in [
        "AMBIGUOUS_TARGET",
...
#[test]
fn result_and_implementation_forbid_secrets_x11_and_desktop_input() {
    let schema = include_str!("../contracts/v1/uix-key-sequence-transition-result.schema.json");
    for marker in [
        "\"applicationInternalEventsDispatched\": { \"const\": true }",
        ...
        "\"textInputSupported\": { \"const\": false }",
        "\"keyHoldSupported\": { \"const\": false }",
        "\"keyRepeatSupported\": { \"const\": false }",
    ...
    for relative_path in [
        "src/adapters/linux/uix_agent_key_sequence_transition.rs",
        "src/modules/linux_uix_key_sequence_transition.rs",
    ] {
...
fn unconfirmed_cli_fails_before_input_target_or_provider_discovery() {
    let error = match cli::run(vec![
        ...
        "apply".to_owned(),
        "--capability".to_owned(),
        "ui.input.key.sequence.transition@1".to_owned(),
        "--target".to_owned(),
        "sessionId=not-a-target".to_owned(),
        "--input".to_owned(),
        "/definitely/not/read-before-confirmation.json".to_owned(),
    ]) {
        Ok(_) => panic!("未确认 key sequence transition 必须优先失败"),
        Err(error) => error,
    };
