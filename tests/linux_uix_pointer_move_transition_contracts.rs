...
//! UIX 单次 pointer_move transition 的输入、结果和安全边界契约回归。
...
fn envelope(data: Value) -> Value {
    json!({
        ...
        "app": "app",
        "verb": "apply",
        "capability": "ui.input.pointer.move.transition@1",
...
#[test]
fn input_and_result_schema_accept_unsettled_move_and_nullable_missing_variant()
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let input_schema = schema(include_str!(
        "../contracts/v1/uix-pointer-move-transition-input.schema.json"
    ))?;
    assert_valid(&input_schema, &input)?;

    let result_schema = schema(include_str!(
        "../contracts/v1/uix-pointer-move-transition-result.schema.json"
    ))?;
...
#[test]
fn input_schema_rejects_unknown_null_float_coordinate_and_non_move_fields()
-> Result<(), Box<dyn std::error::Error>> {
    let input_schema = schema(include_str!(
        "../contracts/v1/uix-pointer-move-transition-input.schema.json"
    ))?;
    ...
    for invalid in [
        ...
        json!({ "coordinateSpace": "client-logical-px", "x": 1, "y": 2, "postcondition": postcondition.clone(), "click": true }),
        json!({ "coordinateSpace": "client-logical-px", "x": 1, "y": 2, "postcondition": postcondition.clone(), "button": "left" }),
        json!({ "coordinateSpace": "client-logical-px", "x": 1, "y": 2, "postcondition": postcondition.clone(), "intervalMs": 100 }),
        json!({ "coordinateSpace": "client-logical-px", "x": 1, "y": 2, "postcondition": postcondition.clone(), "interpolation": true }),
        json!({ "coordinateSpace": "client-logical-px", "x": 1, "y": 2, "postcondition": postcondition.clone(), "drag": true }),
        json!({ "coordinateSpace": "client-logical-px", "x": 1, "y": 2, "postcondition": postcondition.clone(), "scroll": true }),
...
fn confirmation_dispatch_and_semantic_wait_keep_the_required_source_order() {
    let module = include_str!("../src/modules/linux_uix_pointer_move_transition.rs");
    let adapter = include_str!("../src/adapters/linux/uix_agent_pointer_move_transition.rs");
    let Some(confirmation) = module.find("if !confirmed") else {
        panic!("pointer move transition 必须先检查 confirmation");
    };
    let Some(input) = module.find("UixPointerMoveTransitionInput::parse") else {
        panic!("pointer move transition 必须解析输入");
    };
    let Some(provider) = module.find(".perform(session_id") else {
        panic!("pointer move transition 必须通过 provider port 执行动作");
    };
    assert!(confirmation < input && input < provider);

    let Some(dispatch) = adapter.find("perform_targetless_with_request_id") else {
        panic!("Adapter 必须先执行单次 pointer_move");
    };
    let Some(wait) = adapter.find("execute_wait_from_baseline") else {
        panic!("Adapter 必须在 move dispatch 后执行 semantic wait");
    };
...
fn result_and_implementation_forbid_foreground_desktop_input_and_fallback() {
    let schema = include_str!("../contracts/v1/uix-pointer-move-transition-result.schema.json");
    let module = include_str!("../src/modules/linux_uix_pointer_move_transition.rs");
    let adapter = include_str!("../src/adapters/linux/uix_agent_pointer_move_transition.rs");
    ...
    assert!(schema.contains("\"clickSupported\": { \"const\": false }"));
    assert!(schema.contains("\"independentButtonOwnership\": { \"const\": false }"));
    assert!(schema.contains("\"interpolationSemantics\": { \"const\": false }"));
    assert!(schema.contains("\"dragSupported\": { \"const\": false }"));
    assert!(schema.contains("\"scrollSupported\": { \"const\": false }"));
