//! 协调确认、完整普通左键点击序列与提交后的语义条件同步。

use serde_json::{Map, Value, json};

use crate::{
    adapters::linux::uix_agent::{
        self, ElementWaitFailure, PointerClickSequenceFailure,
        PointerClickSequenceTransitionFailure, PointerClickSequenceTransitionFailureSource,
        PointerClickSequenceTransitionOutcome, WindowActionFailure,
    },
    capabilities,
    components::uix_pointer_click_sequence_transition_contract::UixPointerClickSequenceTransitionInput,
    domain::{AppControlError, AppResult},
    modules::{uix_element_wait, uix_window},
};

const CAPABILITY: &str = capabilities::UI_INPUT_POINTER_CLICK_SEQUENCE_TRANSITION;
const PROVIDER: &str = "uix-agent-v1";

/// Module 与系统 Adapter 之间只传递已验证的点击序列和中立结果。
trait UixPointerClickSequenceTransitionPort {
    fn perform(
        &self,
        session_id: &str,
        input: &UixPointerClickSequenceTransitionInput,
    ) -> Result<PointerClickSequenceTransitionOutcome, PointerClickSequenceTransitionFailure>;
}

struct SystemUixPointerClickSequenceTransition;

impl UixPointerClickSequenceTransitionPort for SystemUixPointerClickSequenceTransition {
    fn perform(
        &self,
        session_id: &str,
        input: &UixPointerClickSequenceTransitionInput,
    ) -> Result<PointerClickSequenceTransitionOutcome, PointerClickSequenceTransitionFailure> {
        uix_agent::perform_pointer_click_sequence_transition(session_id, input)
    }
}

/// 对精确协作式窗口执行一次已确认的完整普通左键点击序列，并等待语义后置条件。
pub(crate) fn perform(session_id: &str, confirmed: bool, value: &Value) -> AppResult<Value> {
    perform_with(
        &SystemUixPointerClickSequenceTransition,
        session_id,
        confirmed,
        value,
    )
...
fn perform_with(
    port: &impl UixPointerClickSequenceTransitionPort,
    session_id: &str,
    confirmed: bool,
    value: &Value,
) -> AppResult<Value> {
    ...
    if !confirmed {
        return Err(AppControlError::new(
            ...
            "UIX pointer click sequence transition requires explicit confirmation.",
        ));
    }
    let input = UixPointerClickSequenceTransitionInput::parse(value)
    ...
    let Value::Object(mut result) = json!({
        ...
        unreachable!("pointer click sequence transition 观察投影固定为 JSON 对象");
    };
    let Value::Object(execution) = json!({
        ...
        unreachable!("pointer click sequence transition 执行投影固定为 JSON 对象");
    };
...
fn transition_error(
    failure: PointerClickSequenceTransitionFailure,
    input: &UixPointerClickSequenceTransitionInput,
) -> AppControlError {
    if let PointerClickSequenceTransitionFailureSource::Postcondition(
    ...
    ) = failure.source
    {
        ...
        return AppControlError::with_details(
            "AMBIGUOUS_TARGET",
            "The UIX pointer click sequence transition postcondition matched multiple elements after the complete sequence; automatic retry is prohibited.",
            details,
        );
    }
    if failure.accepted_may_have_occurred {
        ...
        return AppControlError::with_details(
            "OUTCOME_UNKNOWN",
            "The UIX pointer click sequence transition may have executed without a trusted semantic postcondition; automatic retry is prohibited.",
            failure_details(&failure, input, reason, true, None),
        );
    }

    match failure.source {
        PointerClickSequenceTransitionFailureSource::Transport(source) => {
        ...
        PointerClickSequenceTransitionFailureSource::NegotiationUnavailable => {
            AppControlError::with_details(
                "CAPABILITY_UNAVAILABLE",
                "The authenticated UIX Agent does not publish the complete pointer click sequence and semantic revision synchronization surface.",
                failure_details(&failure, input, "agent-preflight-missing", false, None),
            )
        }
        PointerClickSequenceTransitionFailureSource::Sequence(sequence) => {
            safe_sequence_error(sequence, &failure, input)
        }
        PointerClickSequenceTransitionFailureSource::Postcondition(_) => {
            AppControlError::with_details(
                "OUTCOME_UNKNOWN",
                "The complete UIX pointer click sequence lost its trusted semantic postcondition; automatic retry is prohibited.",
...
fn safe_sequence_error(
    sequence: PointerClickSequenceFailure,
    failure: &PointerClickSequenceTransitionFailure,
    input: &UixPointerClickSequenceTransitionInput,
) -> AppControlError {
    match sequence.source {
        ...
        WindowActionFailure::UnsupportedAction => AppControlError::with_details(
            "CAPABILITY_UNAVAILABLE",
            "The authenticated UIX Agent does not publish the requested pointer click sequence actions.",
        ...
        WindowActionFailure::StaleRevision => AppControlError::with_details(
            "STALE_SESSION",
            "The UIX window changed before pointer click sequence transition dispatch.",
        ...
        WindowActionFailure::NotPresentable => AppControlError::with_details(
            ...
            "The UIX application window is not presentable for pointer click sequence transition dispatch.",
        ...
        | WindowActionFailure::OutcomeUnknown => AppControlError::with_details(
            "OUTCOME_UNKNOWN",
            "The UIX pointer click sequence transition may have partially executed; automatic retry is prohibited.",
            failure_details(
                failure,
                input,
...
fn failure_details(
    ...
    details.insert(
        "clicksRequested".to_owned(),
        Value::from(input.clicks_requested()),
    );
    details.insert(
...
mod tests {
    ...
    use crate::adapters::linux::uix_agent::{
        ElementWaitOutcome, Failure, NodeRecord, PointerClickSequenceOutcome, RectRecord, Snapshot,
    };
    ...
    impl UixPointerClickSequenceTransitionPort for FixturePort {
        fn perform(
            &self,
            _: &str,
            _: &UixPointerClickSequenceTransitionInput,
        ) -> Result<PointerClickSequenceTransitionOutcome, PointerClickSequenceTransitionFailure>
        {
            match &self.result {
                Ok(outcome) => Ok(PointerClickSequenceTransitionOutcome {
    ...
    fn success(settled: bool) -> PointerClickSequenceTransitionOutcome {
        let node = NodeRecord {
        ...
        PointerClickSequenceTransitionOutcome {
            sequence: PointerClickSequenceOutcome {
                revision: 7,
    ...
    fn pre_dispatch_failure(source: WindowActionFailure) -> PointerClickSequenceTransitionFailure {
        PointerClickSequenceTransitionFailure {
            source: PointerClickSequenceTransitionFailureSource::Sequence(
    ...
    fn confirmation_precedes_input_target_and_provider() {
        ...
        let Err(error) = perform_with(&port, "not-a-target", false, &Value::Null) else {
            panic!("未确认 pointer click sequence transition 必须优先失败");
        };
    ...
    fn complete_sequence_can_succeed_with_trusted_unsettled_postcondition() {
        ...
        let Ok(result) = perform_with(&port, target, true, &input()) else {
            panic!("可信后置条件必须允许 settled=false 的完整点击序列成功");
        };
        assert_eq!(result["sequenceSettled"], false);
        assert_eq!(result["clicksAccepted"], 2);
        ...
        assert_eq!(result["safety"]["sameAuthenticatedConnectionUsed"], true);
        assert_eq!(result["safety"]["desktopInputInjected"], false);
        assert_eq!(result["safety"]["doubleClickSupported"], false);
        ...
        let Ok(schema) = serde_json::from_str::<Value>(include_str!(
            ...
            panic!("pointer click sequence transition 结果 schema 必须成功解析");
        };
        assert!(
            jsonschema::draft202012::validate(&schema, &envelope).is_ok(),
            "pointer click sequence transition 结果必须匹配公开 schema"
        );
    }

    #[test]
    fn partial_sequence_is_unknown_and_non_retryable() {
        let Ok(parsed) = UixPointerClickSequenceTransitionInput::parse(&input()) else {
            panic!("测试输入必须有效");
        };
        let failure = PointerClickSequenceTransitionFailure {
            source: PointerClickSequenceTransitionFailureSource::Sequence(
                PointerClickSequenceFailure {
    ...
    fn ambiguous_postcondition_is_specific_and_non_retryable() {
        ...
        let failure = PointerClickSequenceTransitionFailure {
            source: PointerClickSequenceTransitionFailureSource::Postcondition(
