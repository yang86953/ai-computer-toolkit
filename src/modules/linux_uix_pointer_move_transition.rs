//! 协调确认、单次 pointer_move 与提交后语义条件同步。

use serde_json::{Value, json};

use crate::{
    adapters::linux::uix_agent::{
        self, ElementWaitFailure, PointerMoveTransitionFailure, PointerMoveTransitionFailureSource,
        PointerMoveTransitionOutcome, WindowActionFailure,
    },
    capabilities,
    components::uix_pointer_move_transition_contract::UixPointerMoveTransitionInput,
    domain::{AppControlError, AppResult},
    modules::{uix_element_wait, uix_window},
};

const CAPABILITY: &str = capabilities::UI_INPUT_POINTER_MOVE_TRANSITION;
const PROVIDER: &str = "uix-agent-v1";

trait UixPointerMoveTransitionPort {
    fn perform(
        &self,
        session_id: &str,
        input: &UixPointerMoveTransitionInput,
    ) -> Result<PointerMoveTransitionOutcome, PointerMoveTransitionFailure>;
}

struct SystemUixPointerMoveTransition;

impl UixPointerMoveTransitionPort for SystemUixPointerMoveTransition {
    fn perform(
        &self,
        session_id: &str,
        input: &UixPointerMoveTransitionInput,
    ) -> Result<PointerMoveTransitionOutcome, PointerMoveTransitionFailure> {
        uix_agent::perform_pointer_move_transition(session_id, input)
    }
}

/// 对精确协作式窗口执行一次已确认的应用内 pointer_move，并等待语义后置条件。
pub(crate) fn perform(session_id: &str, confirmed: bool, value: &Value) -> AppResult<Value> {
    perform_with(
        &SystemUixPointerMoveTransition,
        session_id,
        confirmed,
        value,
    )
...
fn perform_with(
    port: &impl UixPointerMoveTransitionPort,
    session_id: &str,
    confirmed: bool,
    value: &Value,
) -> AppResult<Value> {
    ...
    if !confirmed {
        return Err(AppControlError::new(
            ...
            "UIX pointer move transition requires explicit confirmation.",
        ));
    }
    let input = UixPointerMoveTransitionInput::parse(value)
    ...
    let Value::Object(mut result) = json!({
        "ok": true,
        "contractVersion": "act/ui-pointer-move-transition/v1",
        ...
        unreachable!("pointer move transition 观察投影固定为 JSON 对象");
    };
    let Value::Object(execution) = json!({
        ...
        "effectObservation": "semantic-postcondition-after-pointer-move-dispatch-no-causal-claim",
        ...
        unreachable!("pointer move transition 执行投影固定为 JSON 对象");
    };
...
fn transition_error(
    failure: PointerMoveTransitionFailure,
    input: &UixPointerMoveTransitionInput,
) -> AppControlError {
    if let PointerMoveTransitionFailureSource::Postcondition(ElementWaitFailure::Ambiguous {
        match_count,
    }) = failure.source
    {
        return AppControlError::with_details(
            "AMBIGUOUS_TARGET",
            "The UIX pointer move transition postcondition matched multiple elements after the pointer move may have been dispatched; automatic retry is prohibited.",
            failure_details(
                &failure,
                input,
    ...
    if failure.accepted_may_have_occurred {
        return AppControlError::with_details(
            "OUTCOME_UNKNOWN",
            "The UIX pointer move transition lost a trusted semantic postcondition after the pointer move may have been dispatched; automatic retry is prohibited.",
            failure_details(&failure, input, "trusted-postcondition-lost", true, None),
        );
    }
    match failure.source {
        PointerMoveTransitionFailureSource::Transport(source) => {
        ...
        PointerMoveTransitionFailureSource::NegotiationUnavailable => {
            AppControlError::with_details(
                "CAPABILITY_UNAVAILABLE",
                "The authenticated UIX Agent does not publish pointer_move plus semantic revision synchronization for a pointer move transition.",
                failure_details(&failure, input, "agent-preflight-missing", false, None),
            )
        }
        PointerMoveTransitionFailureSource::Action(source) => {
            safe_action_error(source, &failure, input)
        }
        PointerMoveTransitionFailureSource::Postcondition(_) => AppControlError::with_details(
            "OUTCOME_UNKNOWN",
            "The UIX pointer move may have been dispatched without a trusted postcondition.",
...
fn safe_action_error(
    ...
    match source {
        ...
        WindowActionFailure::UnsupportedAction => AppControlError::with_details(
            "CAPABILITY_UNAVAILABLE",
            "The authenticated UIX Agent does not publish application-internal pointer_move.",
            failure_details(
                failure,
                input,
        ...
        WindowActionFailure::StaleRevision => AppControlError::with_details(
            "STALE_SESSION",
            "The UIX window changed before pointer move dispatch.",
        ...
        WindowActionFailure::Forbidden => AppControlError::with_details(
            "PERMISSION_DENIED",
            "The UIX application policy forbids the pointer move.",
        ...
        WindowActionFailure::NotPresentable => AppControlError::with_details(
            ...
            "The UIX application window is not presentable for the pointer move.",
        ...
        | WindowActionFailure::OutcomeUnknown => AppControlError::with_details(
            "OUTCOME_UNKNOWN",
            "The UIX pointer move may have been dispatched without a trusted postcondition; automatic retry is prohibited.",
            failure_details(
                failure,
                input,
...
mod tests {
    ...
    struct FixturePort {
        result: Result<PointerMoveTransitionOutcome, PointerMoveTransitionFailure>,
    }

    impl UixPointerMoveTransitionPort for FixturePort {
        fn perform(
            &self,
            _: &str,
            _: &UixPointerMoveTransitionInput,
        ) -> Result<PointerMoveTransitionOutcome, PointerMoveTransitionFailure> {
            match &self.result {
                Ok(outcome) => Ok(PointerMoveTransitionOutcome {
    ...
    fn input() -> Value {
        json!({
            ...
            "y": 20.0,
            "postcondition": {
                "selector": { "automationId": "hover-target" },
                "condition": "unique"
            },
            "timeoutMs": 1000
        })
    ...
    fn success(settled: bool) -> PointerMoveTransitionOutcome {
        let node = NodeRecord {
            native_id: "7:1".to_owned(),
            parent_native_id: None,
            automation_id: Some("hover-target".to_owned()),
            focused: false,
            role: "status".to_owned(),
            name: "hover-target".to_owned(),
            enabled: true,
            actions: Vec::new(),
            frame: RectRecord {
                x: 0.0,
    ...
    fn confirmation_precedes_input_target_and_provider() {
        ...
        let Err(error) = perform_with(&port, "not-a-target", false, &Value::Null) else {
            panic!("未确认 pointer move transition 必须优先失败");
        };
    ...
    fn pre_dispatch_remote_timeout_preserves_specific_error() {
        let failure = PointerMoveTransitionFailure {
            source: PointerMoveTransitionFailureSource::Action(WindowActionFailure::Transport(
                Failure::Timeout,
            )),
    ...
    #[test]
    fn unsettled_move_can_succeed_when_postcondition_is_trusted() {
        let port = FixturePort {
            result: Ok(success(false)),
        };
        let Ok(result) = perform_with(&port, "s2:w:0123456789abcdef", true, &input()) else {
            panic!("可信后置条件必须允许 settled=false 的 pointer move 成功");
        };
        ...
        assert_eq!(result["safety"]["interpolationSemantics"], false);
    }

    #[test]
    fn postcondition_ambiguity_remains_specific_and_non_retryable() {
        let failure = PointerMoveTransitionFailure {
            source: PointerMoveTransitionFailureSource::Postcondition(
