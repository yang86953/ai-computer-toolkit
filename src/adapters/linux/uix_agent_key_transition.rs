//! UIX Agent 完整按键与提交后语义条件同步的同连接 Adapter。

use std::time::{Duration, Instant};

use crate::components::{
    opaque_id::{OpaqueTargetId, OpaqueTargetKind},
    uix_key_transition_contract::UixKeyTransitionInput,
};
...
const TRANSITION_REQUEST_ID: &str = "act-key-transition";

/// 区分 mutation 前失败、完整按键失败与动作后条件观测失败。
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum KeyTransitionFailureSource {
    Transport(Failure),
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct KeyTransitionFailure {
    pub(crate) source: KeyTransitionFailureSource,
...
impl KeyTransitionFailure {
    fn before_dispatch(source: KeyTransitionFailureSource) -> Self {
        Self {
            source,
    ...
    fn action(source: WindowActionFailure) -> Self {
        Self {
            source: KeyTransitionFailureSource::Action(source),
    ...
    fn after_dispatch(source: ElementWaitFailure, action: ActionOutcome) -> Self {
        Self {
            source: KeyTransitionFailureSource::Postcondition(source),
...
/// 完整按键响应与提交后语义条件观察均可信时的中立事实。
#[derive(Debug)]
pub(crate) struct KeyTransitionOutcome {
    pub(crate) action: ActionOutcome,
    pub(crate) postcondition: ElementWaitOutcome,
}

/// 在一个总 deadline 内只解析一次窗口并完成完整按键与语义条件同步。
pub(crate) fn perform_key_transition(
    target: &str,
    ...
    if OpaqueTargetId::parse(target).map(OpaqueTargetId::kind) != Some(OpaqueTargetKind::Window) {
        return Err(KeyTransitionFailure::before_dispatch(
            KeyTransitionFailureSource::Transport(Failure::Stale),
        ));
    }
    ...
    let window = resolve_until(target, deadline).map_err(|source| {
        KeyTransitionFailure::before_dispatch(KeyTransitionFailureSource::Transport(source))
    })?;
...
fn execute_transition(
    ...
    {
        return Err(KeyTransitionFailure::before_dispatch(
            KeyTransitionFailureSource::NegotiationUnavailable,
        ));
    }
    if super::remaining(client.deadline).is_err() {
        return Err(KeyTransitionFailure::before_dispatch(
            KeyTransitionFailureSource::Transport(Failure::Timeout),
        ));
    }
    ...
    let action = client
        ...
        .map_err(KeyTransitionFailure::action)?;
    ...
    .map_err(|source| KeyTransitionFailure::after_dispatch(source, action))?;
    Ok(KeyTransitionOutcome {
        action,
        postcondition,
    })
}
...
mod tests {
    ...
    fn input() -> UixKeyTransitionInput {
    ...
    fn fixture_client(stream: UnixStream) -> AgentClient {
        AgentClient {
            ...
            key_names: BTreeSet::from(["enter".to_owned()]),
            key_modifiers: BTreeSet::from(["control".to_owned()]),
    ...
    #[test]
    fn missing_wait_or_key_catalog_fails_before_dispatch() {
    ...
    #[test]
    fn key_and_postcondition_share_one_connection() {
        ...
        let server = thread::spawn(move || {
            ...
            assert_eq!(perform["action"]["kind"], "press_key");
            assert!(
                writeln!(
                    reader.get_mut(),
                    "{}",
