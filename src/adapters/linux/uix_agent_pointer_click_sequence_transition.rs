//! UIX Agent 完整普通左键点击序列及提交后语义条件同步的同连接 Adapter。

use std::time::{Duration, Instant};

use crate::components::{
    opaque_id::{OpaqueTargetId, OpaqueTargetKind},
    uix_pointer_click_sequence_transition_contract::UixPointerClickSequenceTransitionInput,
};

use super::{
    AgentClient, ElementWaitFailure, ElementWaitOutcome, Failure, PointerClickSequenceFailure,
    PointerClickSequenceOutcome, WindowRecord, resolve_until,
};

const EXECUTION_RESERVE: Duration = Duration::from_millis(100);

/// 区分 mutation 前失败、点击序列失败与序列后的条件观测失败。
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum PointerClickSequenceTransitionFailureSource {
    Transport(Failure),
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct PointerClickSequenceTransitionFailure {
    pub(crate) source: PointerClickSequenceTransitionFailureSource,
...
impl PointerClickSequenceTransitionFailure {
    fn before_dispatch(source: PointerClickSequenceTransitionFailureSource) -> Self {
        Self {
            source,
    ...
    fn sequence(failure: PointerClickSequenceFailure) -> Self {
        Self {
            source: PointerClickSequenceTransitionFailureSource::Sequence(failure),
            accepted_may_have_occurred: failure.accepted_may_have_occurred
                || failure.clicks_accepted > 0,
            sequence_completed: false,
            sequence_revision: None,
            sequence_presented_revision: None,
            sequence_settled: None,
    ...
    fn after_sequence(source: ElementWaitFailure, sequence: PointerClickSequenceOutcome) -> Self {
        Self {
            source: PointerClickSequenceTransitionFailureSource::Postcondition(source),
...
/// 完整点击序列与提交后语义条件均可信时的中立事实。
#[derive(Debug)]
pub(crate) struct PointerClickSequenceTransitionOutcome {
    pub(crate) sequence: PointerClickSequenceOutcome,
    pub(crate) postcondition: ElementWaitOutcome,
}

/// 在一个总 deadline 与认证连接内执行完整点击序列，随后等待语义后置条件。
pub(crate) fn perform_pointer_click_sequence_transition(
    target: &str,
    ...
    if OpaqueTargetId::parse(target).map(OpaqueTargetId::kind) != Some(OpaqueTargetKind::Window) {
        return Err(PointerClickSequenceTransitionFailure::before_dispatch(
            PointerClickSequenceTransitionFailureSource::Transport(Failure::Stale),
        ));
    }
    ...
    let window = resolve_until(target, deadline).map_err(|source| {
        PointerClickSequenceTransitionFailure::before_dispatch(
...
fn execute_transition(
    ...
    // 首个 click 前一次性预检语义同步、perform 和 click_at 所需的完整协议面。
    ...
    if client.deadline.saturating_duration_since(Instant::now()) < required {
        return Err(PointerClickSequenceTransitionFailure::before_dispatch(
            PointerClickSequenceTransitionFailureSource::Transport(Failure::Timeout),
        ));
    }

    let sequence =
        super::uix_agent_pointer_click_sequence::execute_sequence(client, window, input.sequence())
            .map_err(PointerClickSequenceTransitionFailure::sequence)?;
    // 只有所有 click 完成并返回最终修订后，才允许进入语义条件等待。
    ...
    .map_err(|source| PointerClickSequenceTransitionFailure::after_sequence(source, sequence))?;
    Ok(PointerClickSequenceTransitionOutcome {
        sequence,
        postcondition,
    })
}
...
mod tests {
    ...
    use crate::adapters::linux::uix_agent::WindowActionFailure;

    use super::*;

    fn input() -> UixPointerClickSequenceTransitionInput {
        let Ok(input) = UixPointerClickSequenceTransitionInput::parse(&json!({
            ...
            panic!("测试 pointer click sequence transition 输入必须有效");
        };
        input
    }
    ...
    fn missing_wait_or_click_action_fails_before_first_dispatch() {
        for missing in ["wait", "click_at"] {
            ...
            let Err(failure) = execute_transition(
                ...
                panic!("缺失 {missing} 必须在首个 click 前失败");
            };
            assert_eq!(
                failure.source,
                PointerClickSequenceTransitionFailureSource::NegotiationUnavailable
    ...
    fn complete_sequence_and_postcondition_share_one_connection() {
        ...
        let server = thread::spawn(move || -> Result<(), String> {
                ...
                .enumerate()
            {
                ...
                if request["type"] != "perform"
                    || request["request_id"] != "act-pointer-click-sequence"
        ...
        let Ok(outcome) = execute_transition(
            ...
            panic!("完整点击序列与语义条件必须成功");
        };
        assert_eq!(outcome.sequence.clicks_accepted, 3);
    ...
    fn lost_first_click_reply_is_unknown_and_not_safe_to_retry() {
        ...
        let PointerClickSequenceTransitionFailureSource::Sequence(sequence) = failure.source else {
            panic!("首项响应丢失必须归类为点击序列失败");
        };
        assert_eq!(sequence.source, WindowActionFailure::OutcomeUnknown);
        assert!(failure.accepted_may_have_occurred);
        assert_eq!(failure.clicks_accepted, 0);
        let Ok(server_result) = server.join() else {
            panic!("测试服务器必须正常停止");
