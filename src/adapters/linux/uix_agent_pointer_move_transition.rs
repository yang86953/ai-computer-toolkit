//! UIX Agent 单次 pointer_move 与提交后语义条件同步的同连接 Adapter。

use std::time::{Duration, Instant};

use crate::components::{
    opaque_id::{OpaqueTargetId, OpaqueTargetKind},
    uix_pointer_move_transition_contract::UixPointerMoveTransitionInput,
};
...
const TRANSITION_REQUEST_ID: &str = "act-pointer-move-transition";

/// 区分 mutation 前失败、pointer_move 失败与动作后条件观测失败。
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum PointerMoveTransitionFailureSource {
    Transport(Failure),
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct PointerMoveTransitionFailure {
    pub(crate) source: PointerMoveTransitionFailureSource,
...
impl PointerMoveTransitionFailure {
    fn before_dispatch(source: PointerMoveTransitionFailureSource) -> Self {
        Self {
            source,
    ...
    fn action(source: WindowActionFailure) -> Self {
        Self {
            source: PointerMoveTransitionFailureSource::Action(source),
    ...
    fn after_dispatch(source: ElementWaitFailure, action: ActionOutcome) -> Self {
        Self {
            source: PointerMoveTransitionFailureSource::Postcondition(source),
...
/// pointer_move 响应与提交后语义条件观察均可信时的中立事实。
#[derive(Debug)]
pub(crate) struct PointerMoveTransitionOutcome {
    pub(crate) action: ActionOutcome,
    pub(crate) postcondition: ElementWaitOutcome,
}

/// 在一个总 deadline 内只解析一次窗口并完成一次 pointer_move 与条件同步。
pub(crate) fn perform_pointer_move_transition(
    target: &str,
    ...
    if OpaqueTargetId::parse(target).map(OpaqueTargetId::kind) != Some(OpaqueTargetKind::Window) {
        return Err(PointerMoveTransitionFailure::before_dispatch(
            PointerMoveTransitionFailureSource::Transport(Failure::Stale),
        ));
    }
    ...
    let window = resolve_until(target, deadline).map_err(|source| {
        PointerMoveTransitionFailure::before_dispatch(
...
fn execute_transition(
    ...
    // mutation 前一次性预检语义条件同步与 pointer_move 所需的完整协议面。
    ...
    if super::remaining(client.deadline).is_err() {
        return Err(PointerMoveTransitionFailure::before_dispatch(
            PointerMoveTransitionFailureSource::Transport(Failure::Timeout),
        ));
    }
    ...
    let action = client
        ...
        .map_err(PointerMoveTransitionFailure::action)?;
    ...
    .map_err(|source| PointerMoveTransitionFailure::after_dispatch(source, action))?;
    Ok(PointerMoveTransitionOutcome {
        action,
        postcondition,
    })
}
...
mod tests {
    ...
    fn input() -> UixPointerMoveTransitionInput {
        let Ok(input) = UixPointerMoveTransitionInput::parse(&json!({
            ...
            panic!("测试 pointer move transition 输入必须有效");
        };
        input
    }
    ...
    fn missing_wait_or_pointer_move_fails_before_first_dispatch() {
        ...
        let Err(failure) = execute_transition(
            ...
            panic!("缺失 wait 必须在 pointer_move dispatch 前失败");
        };
        assert_eq!(
            failure.source,
            PointerMoveTransitionFailureSource::NegotiationUnavailable
    ...
    #[test]
    fn move_and_postcondition_share_one_connection_and_keep_unsettled_fact() {
        ...
        let server = thread::spawn(move || {
            ...
            assert_eq!(perform["action"]["kind"], "pointer_move");
        ...
        let Ok(outcome) = execute_transition(
            ...
            panic!("同连接 pointer_move 与语义条件必须成功");
        };
